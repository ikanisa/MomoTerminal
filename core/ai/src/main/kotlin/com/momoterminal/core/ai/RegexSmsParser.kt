...
/**
 * Regex-based SMS parser for Mobile Money transactions.
 * Used as final fallback when AI parsers fail.
 */
@Singleton
class RegexSmsParser @Inject constructor() {
    ...
    private fun extractAmount(body: String): Pair<Double, String> {
        // Try currency-first pattern (GHS 100.00)
        ...
        // Try amount-first pattern (100.00 GHS)
    ...
    private fun extractBalance(body: String): Double? {
        for (pattern in balancePatterns) {
            pattern.find(body)?.let { match ->
                // Handle patterns with or without currency in capture group
