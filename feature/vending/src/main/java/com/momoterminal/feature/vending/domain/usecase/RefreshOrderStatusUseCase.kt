...
class RefreshOrderStatusUseCase @Inject constructor(
    private val repository: VendingRepository
) {
    suspend operator fun invoke(orderId: String): Result<VendingOrder> {
        return repository.refreshOrderStatus(orderId)
    }
}
